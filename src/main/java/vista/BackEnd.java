...
public class BackEnd {
	...
	public BackEnd() {
		try {
			String pepe = Files.lines(Paths.get("res/winCases.txt"), StandardCharsets.UTF_8).map(c -> c.toString())
		...
		;
		Integer[][] harcodeadisimo = new Integer[][] { new Integer[] { 1, 2, 3 }, new Integer[] { 4, 5, 6 },
				new Integer[] { 7, 8, 9 }, new Integer[] { 1, 4, 7 }, new Integer[] { 2, 5, 8 },
				new Integer[] { 3, 6, 9 }, new Integer[] { 1, 5, 9 }, new Integer[] { 3, 5, 7 },
				new Integer[] { 2, 4, 9 }, new Integer[] { 2, 6, 7 }, new Integer[] { 8, 4, 3 },
				new Integer[] { 8, 6, 1 } };

		this.winSituations = Arrays.asList(harcodeadisimo).stream().map(array -> Arrays.asList(array))
	...
	public List<List<Integer>> filterWinConditions(List<List<Integer>> possibleWins, Integer index) {
		return possibleWins.stream().filter(winCondition -> !winCondition.contains(index)).collect(Collectors.toList());
	}
